...
double flowUnitArea(double P1, double T1, double R1, double K1, double P2, double T2 = NULL, double R2 = NULL, double K2 = NULL);

//一个喷管
class ValveSimple:public flowLink
{
public:
	double flowArea;
	ArrayTable flow;
	...
	bool connect(volComponent* connectFrom = nullptr, volComponent* connectTo = nullptr);

	//double get_DMDT() { return DMDT; };
...
class Valve:public flowLink
{
public:

	double valveDiameter;//气阀
	double Open_Timing_Angle;//开启角
	double Close_Timing_Angle;//关闭角
	double rock;//
	double sigma;
